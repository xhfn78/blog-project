...
### 5. 기술적 무결성 (Technical Integrity) - [필수]

**빌드 및 린트 체크:**
- [ ] `npm run lint` 실행 시 에러가 없는가?
- [ ] `npm run build` 시 해당 도구 관련 타입 오류가 없는가?
- [ ] `src/shared/ui` 컴포넌트 임포트 경로가 올바른가? (`@/shared/ui/...`)

**수정 지침:**
- 린트/빌드 에러 발견 시 **즉시 수정**하고 재검사하세요.
- 타입 오류를 무시하기 위해 `// @ts-ignore` 등을 사용하는 것은 최후의 수단으로만 고려하세요.

---

## 📋 수행 작업

### Step 1: 파일 읽기 및 기술 검증
```
1. src/features/tools/tools/[slug]/index.tsx 읽기
2. src/features/tools/tools/[slug]/tool.config.ts 읽기
3. 쉘 명령어로 npm run lint 실행하여 결과 확인
```
